...
class dgmstcn(nn.Module):
    ...
    def __init__(self,
        ...
        for i, cfg in enumerate(ms_cfg):
            ...
            # branches.append(branch)
            branch = nn.Sequential(
                nn.Conv2d(in_channels, branch_c * 2 if use_glu else branch_c, kernel_size=1),  # Use 2*branch_c for GLU
                nn.BatchNorm2d(branch_c * 2 if use_glu else branch_c),
                unit_tcn(branch_c * 2 if use_glu else branch_c, branch_c * 2 if use_glu else branch_c,
                         kernel_size=cfg[0], stride=stride, dilation=cfg[1], norm=None),
                nn.BatchNorm2d(branch_c * 2 if use_glu else branch_c)
            )
            # Add GLU if use_glu is True
            if use_glu:
                branch.add_module('glu', nn.GLU())
            branches.append(branch)
