...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):
        y = self.relu(self.drop_out(self.tcn1(self.gcn1(x))) + self.residual(x))
        return y


class CTRGCN(nn.Module):
    def __init__(self, num_classes=60, num_point=25, num_person=2, graph_cfg=None,  in_channels=2,
                 drop_out=0, adaptive=True, args=None):
        ...
        if self.in_channels != self.base_channels:
            modules = [TCN_GCN_unit(in_channels, base_channel, A, residual=False, adaptive=adaptive)]
        
        '''init block'''
        # layer 2~num_input
        for i in range(1, args.num_init):
            modules.append(TCN_GCN_unit(base_channel, base_channel, A, adaptive=adaptive, kernel_size=5, drop_out=args.dropout_bk))
        ...
        for i in range(0, args.num_in):
            ...
            modules.append(TCN_GCN_unit(in_channels, out_channels, A, adaptive=adaptive, kernel_size=args.tkernel_in, stride=stride, drop_out=args.dropout_bk))
