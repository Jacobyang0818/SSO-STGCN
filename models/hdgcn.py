...
from .hdgcntools.tools import get_groups
...
class EdgeConv(nn.Module):
    ...
    def forward(self, x, dim=4):  # N, C, T, V
        ...
        # 原本：x = x.max(dim=-1, keepdim=False)[0]
        x, _ = torch.max(x, dim=-1, keepdim=False)

        if dim == 3:
            pass
        else:
            # 原本：x = repeat(x, 'n c v -> n c t v', t=T)
    ...
    def get_graph_feature(self, x, k, idx=None):
        ...
        # 原本：feature = rearrange(x, 'n v c -> (n v) c')[idx, :]
        feature = x.permute(0, 2, 1).contiguous().view(N * V, C)[idx, :]
        
        # 原本：feature = feature.view(N, V, k, C)
        feature = feature.view(N, V, k, C)

        # 原本：x = repeat(x, 'n v c -> n v k c', k=k)
        x = x.unsqueeze(2).expand(-1, -1, k, -1)

        feature = torch.cat((feature - x, x), dim=3)
        
        # 原本：feature = rearrange(feature, 'n v k c -> n c v k')
...
class HDGCN(nn.Module):
    def __init__(self, num_classes=60, num_point=25, num_person=2, graph='..hdgcngarph.Graph', graph_args=dict(labeling_mode='spatial', CoM=1), in_channels=3,
        ...
        if graph is None:
            ...
            from .hdgcntools.graph import Graph
        ...
        self.fc = nn.Linear(base_channels*4, self.num_class)
    ...
    def forward(self, x):
        ...
        # 原本：x = rearrange(x, 'n c t v m -> n (m v c) t')
        x = x.view(N, M * V * C, T)
        
        x = self.data_bn(x)

        # 原本：x = rearrange(x, 'n (m v c) t -> (n m) c t v', m=M, v=V)
        ...
        # 原本：x = x.view(N, M, c_new, -1).mean(3).mean(1)
        c_new = x.size(1)
